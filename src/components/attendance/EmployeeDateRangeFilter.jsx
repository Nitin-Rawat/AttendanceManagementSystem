...
const EmployeeDateRangeFilter = ({
  ...
  const handleFetchEmployeeData = async () => {
    ...
    try {
      ...
      const employeeLogs = await AttendanceService.fetchAttendanceLogs(
        employeeId,
        startDate,
        endDate
      );
  ...
  const handleExportEmployeeData = async () => {
    ...
    try {
      ...
      // Fill in missing dates but keep whatever data we have
      const completeLogs = AttendanceService.fillMissingDates(
        employeeLogs,
        currentEmployeeId,
        employeeDataDateRange.start,
      ...
      // Export the data to CSV, even if some dates are empty
